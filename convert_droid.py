...
def convert_dataset(path, args):
    ...
    # find a file that ends with "_errors.json" in the same directory as the path
    error_file = glob.glob(os.path.join(os.path.dirname(path), "*_errors.json"))
    if len(error_file) == 0:
        raise ValueError("No error file found")
    error_file = error_file[0]
    # load the error file
    error_dict = json.load(open(error_file))
    k = list(error_dict.keys())[0]
    rotation_error_left_wrist = error_dict[k]["rotation_error_left_wrist"]
    rotation_error_right_wrist = error_dict[k]["rotation_error_right_wrist"]
    if (rotation_error_left_wrist + rotation_error_right_wrist) > 180:
        flip_wrist_flag = True
    else:
        flip_wrist_flag = False

    recording_folderpath = os.path.join(os.path.dirname(path), "recordings", "SVO")
    camera_kwargs = dict(
        # hand_camera=dict(image=True, concatenate_images=False, resolution=(args.imsizew, args.imsizeh), resize_func="cv2"),
        # varied_camera=dict(image=True, concatenate_images=False, resolution=(args.imsizew, args.imsizeh), resize_func="cv2"),
        hand_camera=dict(image=True, depth=False, pointcloud=True, concatenate_images=False, resolution=(args.imsizew, args.imsizeh)),
        varied_camera=dict(image=True, depth=False, pointcloud=True, concatenate_images=False, resolution=(args.imsizew, args.imsizeh)),
    )
    camera_reader = RecordedMultiCameraWrapper(recording_folderpath, camera_kwargs, flip_wrist_flag)

    output_path = os.path.join(os.path.dirname(path), "trajectory_im_pcd.h5")
    ...
    # remove all ".h5" files that are not "trajectory.h5" in the same directory as the path
    for f in glob.glob(os.path.join(os.path.dirname(path), "*.h5")):
        if "trajectory.h5" not in f:
            os.remove(f)
    ...
    if "camera" not in f["observation"]:
        ...
        # f["observation/camera"].create_group("depth")
        f["observation/camera"].create_group("pointcloud")
    image_grp = f["observation/camera/image"]
    # depth_grp = f["observation/camera/depth"]
    pointcloud_grp = f["observation/camera/pointcloud"]
    ...
    hand_cam_ids = []
    varied_cam_ids = []
    # since camera_ids were wrong when collecting data (in parameters.py, id should be 22008760, but used 20120598 instead.
    # so need to manually change it to the correct id)
    # f["observation"]["camera_type"].setdefault("20120598", f["observation"]["camera_type"].pop("22008760", None))
    # since camera_ids were wrong when collecting data (in parameters.py, id should be 22008760, but used 24400334 instead.
    # so need to manually change it to the correct id)
    # f["observation"]["camera_type"].setdefault("22008760", f["observation"]["camera_type"].pop("24400334", None))
    # f["observation"]["camera_type"].setdefault("20120598.", f["observation"]["camera_type"].pop("20120598", None))
    # f["observation"]["camera_type"].setdefault("20120598.", f["observation"]["camera_type"].pop("24400334", None))
    # f["observation"]["camera_type"].setdefault("18026681.", f["observation"]["camera_type"].pop("18026681", None))
    # f["observation"]["camera_type"].setdefault("13919467.", f["observation"]["camera_type"].pop("13919467", None))
    ...
    # cam_depth_data = {cam_name: [] for cam_name in IMAGE_NAME_TO_CAM_KEY_MAPPING.keys()}
    cam_pointcloud_data = {cam_name: [] for cam_name in IMAGE_NAME_TO_CAM_KEY_MAPPING.keys() if "right" not in cam_name}
    ...
    for index in range(demo_len):
        ...
        for cam_name in IMAGE_NAME_TO_CAM_KEY_MAPPING.keys():
            if camera_obs is None:
                im = np.zeros((args.imsizeh, args.imsizew, 3))

            else:
                im_key = IMAGE_NAME_TO_CAM_KEY_MAPPING[cam_name]
                im = camera_obs["image"][im_key][:, :, :3]

            # perform bgr_to_rgb operation
            im = im[:,:,::-1]
            
            cam_data[cam_name].append(im.astype(np.uint8))
            if "right" not in cam_name:
                if camera_obs is not None:
                    # cam_depth_data[cam_name].append(camera_obs["depth"][im_key].copy())
                    cam_pointcloud_data[cam_name].append(camera_obs["pointcloud"][im_key][..., :3].copy())
                else:
                    # fill with nans
                    # cam_depth_data[cam_name].append(np.full((args.imsizeh, args.imsizew), np.nan))
                    cam_pointcloud_data[cam_name].append(np.full((args.imsizeh, args.imsizew, 3), np.nan))
    ...
    for cam_name in cam_data.keys():
        ...
        if "right" not in cam_name:
            # depth_grp.create_dataset(cam_name, data=np.array(cam_depth_data[cam_name]), compression="gzip")
            pointcloud_grp.create_dataset(cam_name, data=np.array(cam_pointcloud_data[cam_name]), compression="gzip")
    ...
    cam_reader_svo = RecordedMultiCameraWrapper(svo_path, camera_kwargs, flip_wrist_flag)
